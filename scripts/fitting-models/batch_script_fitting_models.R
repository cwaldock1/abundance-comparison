...
length_i <- ncol(rls_abun_fitting)-3

Nodes <- c('@RunAsMultiple,@NodeSet_16')

file_out <- c()
for(i in 1:length_i){
  file_out[i] <- paste0('%_shared%R-latest%r.bat %conor%abundance-comparison%scripts%fitting-models%fitting_models.R',
                        ' ', 
                        i)}

file_out <- gsub("%","\\", file_out, fixed=TRUE)

fileConn<-file("fit_models.bat")
