...
targets = list(Amae_rel_mean = c(1,0,3),
               Dintercept    = c(0, -5, 20), 
               Dslope        = c(1,  0, 2), 
               Dpearson      = c(1,  0, 1), 
               Dspearman     = c(1,  0, 1), 
...
bbs_basic <- all_assessments_relative %>% 
  ...
  plot_trait_models(input_data = ., 
                    directory = paste0('figures-R2/species-performance-figures/', unique(.$cross_validation), '/'), 
...
# 
# pdf('figures-R2/species-performance-figures/abun-data-freq-effects/basic.pdf', width = 6, height = 6)
...
best_model_assessments %>% 
  ...
  nest() %>% 
  .[1,] %>% 
  mutate(test = purrr::map(data, ~interaction_trait_plots_R2(., 
                                                          directory = 'figures-R2/species-performance-figures/trait-interaction-models/', 
