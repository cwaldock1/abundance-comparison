...
all_assessments <- all_assessments %>% select(-family_grouped_simple, -family_grouped, -family, 
  ...
  select(dataset, cross_validation, cross_validation_2, fitted_model, abundance_response, plot_level, species_name, 
         Amae_rel_mean, Dintercept, Dslope, Dpearson, Dspearman, Pdispersion, Evaluation_number, Evaluation_message) %>% 
...
# saveRDS(best_models, file = 'results/overall_best_models_R2.RDS')

# plot observed vs. predicted abundances for all model types ----

# results are downloaded from the server

predictions_all <- list.files('results/predictions_all_R2/bind', full.names = T)

model_type <- c('bbs_basic_all', 'bbs_cv_all', 'rls_basic_all', 'rls_cv_all')

validation_all_data_list <- list()
for(model in 1:length(model_type)){
      ...
      # validation_all_data
      validation_all_data_list[[model]] <- validation_data
      
}

for(i in 1:length(validation_all_data_list)){
      
  dir.create('figures-R2/model-prediction-figures/validation_all_models_plot_level', recursive = T)
  
      # create plots 
      observed_predicted_plot(input_data = validation_all_data_list[[i]], 
                              rescale = T, 
                              model_level = 'plot_level', # options are fitted_model or plot_level
                              directory   = 'figures-R2/model-prediction-figures/validation_all_models_plot_level',
                              name        = unique(paste(validation_all_data_list[[i]]$dataset, validation_all_data_list[[i]]$cross_validation, sep = '-')), 
                              width = 8000, 
                              height = 8000, 
                              nbins = 30, 
                              upper_limit = 1.5, 
                              option = ifelse(unique(validation_all_data_list[[i]]$dataset) == 'rls', 'viridis', 3))
...
for(i in 1:length(model_type)){
  ...
  # remove species with identical observations and predictions (cannot fit a model here)
  if(length(which(sapply(validation_data$validation_observed_mean, function(x) length(unique(x))==1)))!=0){
    validation_data <- validation_data[-which(sapply(validation_data$validation_observed_mean, function(x) length(unique(x))==1)),]
  }else{print('no removal of observations for', i)}
  if(length(which(sapply(validation_data$validation_predict_mean, function(x) length(unique(x))==1)))!=0){
    validation_data <- validation_data[-which(sapply(validation_data$validation_predict_mean, function(x) length(unique(x))==1)),]
  }else{print('no removal of predictions for', i)}
...
for(i in 1:length(validation_data_list)){
  
  # create plots 
  force(observed_predicted_plot(input_data = validation_data_list[[i]], 
                          rescale = T, 
                          model_level = 'plot_level', # options are fitted_model or plot_level
                          directory   = 'figures-R2/model-prediction-figures/validation_best_models_plot_level',
                          name        = unique(paste(validation_data_list[[i]]$dataset, validation_data_list[[i]]$cross_validation, sep = '-')), 
                          width = 8000, 
                          height = 8000, 
                          nbins = 30,
                          option = ifelse(unique(validation_data_list[[i]]$dataset) == 'rls', 'viridis', 3)))
  
  force(observed_predicted_plot(input_data = validation_data_list[[i]], 
                          rescale = T, 
                          model_level = 'aggregated', # options are fitted_model or plot_level
                          directory   = 'figures-R2/model-prediction-figures/validation_best_models_aggregated',
                          name        = unique(paste(validation_data_list[[i]]$dataset, validation_data_list[[i]]$cross_validation, sep = '-')), 
                          width = 1200, 
                          height = 1200, 
                          nbins = ifelse(unique(validation_data_list[[i]]$dataset) == 'rls', 30, 30), 
                          option = ifelse(unique(validation_data_list[[i]]$dataset) == 'rls', 'viridis', 3)))
...
for(i in 1:length(model_type)){
  ...
  summary_evals[[i]] <- evaluations %>% 
    select(dataset, cross_validation, metrics, paste0(metrics, '_rescaled')) %>% 
    #select(-Psd, -Psd_rescaled, -Armse, -Armse_rescaled) %>% 
    pivot_longer(., cols = c(metrics, paste0(metrics, '_rescaled'))) %>% 
    group_by(name) %>% 
  ...
  require(patchwork)
  dir <- 'figures-R2/model-prediction-figures/validation_best_models_aggregated_intercept_slope'
...
# summarise non-rescaled and rescaled values for all metrics for supporting Table 2 
summary_evals_2 <- do.call(rbind, summary_evals) %>% 
  mutate(rescaled = ifelse(grepl('_rescaled',name), 'rescaled', 'normal'), 
  ...
  unnest() %>% 
  select(dataset, cross_validation, name, 
         IQR.25_normal, median_value_normal, IQR.75_normal, 
         IQR.25_rescaled, median_value_rescaled, IQR.75_rescaled) %>% 
  mutate_at(., c('IQR.25_normal','median_value_normal', 'IQR.75_normal', 
                 'IQR.25_rescaled', 'median_value_rescaled', 'IQR.75_rescaled'), function(x) round(x, 2))

writexl::write_xlsx(summary_evals_2, path = 'figures-R2/model-prediction-figures/validation_best_models_aggregated/aggregate_summaries.xlsx')
